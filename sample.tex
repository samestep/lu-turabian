...
\item Editor, translator, or compiler \ul{in addition to} of
...
\item Book published
  electronically.\autocite{Turabian}\textsuperscript{,}\footnote{Note: if the
...
\item Article in a print
  journal.\autocite[639]{Turabian}\textsuperscript{,}\footnote{Note: if the
  journal has more than one issue per year---i.e., (April 1998)---put the month
  before the date.}
\item Article in an online
  journal.\autocite{Turabian}\textsuperscript{,}\footnote{For a journal article
  consulted online, include an access date and a URL. For articles that include
  a DOI, form the URL by appending the DOI to http://dx.doi.org/ rather than
  using the URL in your address bar. The DOI for the article in the Brown
